...
public final class OperatorBlocks {
  ...
  private static BlockModel get0Block() {
    BlockModel block = new BlockModel();
    block.setColor("#FE4E66");
    block.setBlockType(BlockModel.Type.number);
    block.setDragAllowed(true);
    block.setHolderName("Operator");
    block.setReplacerKey("0");
    block.setReturns(new String[] {"int"});
    ...
    ImportTag intImport = new ImportTag();
    intImport.setImportClass("java.lang.Integer");

    tag.setAdditionalTags(new AdditionalCodeHelperTag[] {intImport});
    block.setTags(tag);
